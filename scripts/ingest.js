...
//   - "dauerhaft beobachten": as of the Always-On Market Server
//     (server/index.js), this GitHub Actions path is no longer the only
//     source of updates — see docs/ALWAYS_ON_SERVER.md. It remains a
//     periodic, git-committed fallback/backstop, independent of whether
//     the always-on server happens to be running.
//
// Reuses marketBrain.js, events.js, and server/lib/marketStateStore.js as-is
// — this script computes nothing of its own, and persists state using
// EXACTLY the same shape/logic the Always-On Server uses (Daniel's explicit
// "keine zweite parallele Trading-/Market-Engine bauen").
// ---------------------------------------------------------------------------
...
const store = require('../server/lib/marketStateStore.js');

const MARKET_DATA_PATH = path.join(__dirname, '..', 'data', 'market.json');
...
function main() {
  ...
  const prevState = store.readLatestState();
  ...
  const persisted = store.toPersistedState(brain);
  persisted.patternIds = patternIds;

  store.writeLatestState(persisted);
