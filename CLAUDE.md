...
- DG Trading Brain V1 (v0.23.0, "Module 11" in `marketBrain.js`): the first real trading-interpretation layer, built directly from Daniel's own "DG TRADING BRAIN V1" prompt — HTF Context (Weekly → Daily → 4H → 1H), Structure/FVG/Order Block now computed per timeframe (reusing the exact same detectors as the legacy H1 pipeline, no second engine), a regelbasiertes POI Ranking (0-100, explainable `reasons`), a Targets engine, and a Market Report — exposed via `GET /api/brain/XAUUSD` and a new "DG Trading Brain V1" dashboard card (only populated when an Always-On Server is connected; honestly empty otherwise). Explicitly labeled **V1 draft** everywhere, not Daniel's finished DG rules — see the new 🟡 V1 DRAFT chapters (1 HTF Bias, 2 Liquidity, 3 Premium/Discount, 4 Order Block, 5 Valid FVG) in `rules/strategy.md`. No trade execution, no entry automation, no risk engine, no news engine, no M1/M5 — status is one of WAIT/WATCH BUY/WATCH SELL/BULLISH SCENARIO/BEARISH SCENARIO, never BUY/SELL READY.
