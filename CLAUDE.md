...
- DG Trading Brain V1 (v0.23.1, "Module 11" in `marketBrain.js`, built from Daniel's "DG TRADING BRAIN V1" prompt): the Market Facts layer is real and live — HTF Context (Weekly → Daily → 4H → 1H), Structure/FVG/Order Block now computed per timeframe (reusing the exact same detectors as the legacy H1 pipeline, no second engine), Liquidity extended with Previous Day High/Low and swing levels, Premium/Discount per timeframe — exposed via `GET /api/brain/XAUUSD` and a new "DG Trading Brain V1" dashboard card (only populated when an Always-On Server is connected; honestly empty otherwise). The **interpretation layer** (HTF Bias, POI quality/score, Target priority, Market Report status) is architecturally ready but gated behind `DG_RULES_DEFINED` (same mechanism `computeDecisionEngine()` already used) — every one of those fields is the literal string `AWAITING_DG_RULE` (never a guessed bullish/bearish/score/status) until Daniel actually defines the matching `rules/strategy.md` chapter (1 HTF Bias, 4 Order Block, 5 Valid FVG — all still 🔴 TODO, untouched by this build). No trade execution, no entry automation, no risk engine, no news engine, no M1/M5, never BUY/SELL READY.
