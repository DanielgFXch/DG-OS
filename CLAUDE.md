...
- System Status card (v0.21.0): shows Version, Last Market Update, Timezone, Data Age, Data Status (LIVE/DELAYED/STALE/NO_DATA), and Market Source at the top of the dashboard. `computeDataFreshness()` (`marketBrain.js`) derives status purely from the actual last-update timestamp against thresholds tied to DG OS's own known feed intervals (WS heartbeat vs. 15-min cron) — never claims LIVE off a stale timestamp, and drives the header badge, ticker, and status card from one shared value so they can't disagree. Full writeup: `docs/MARKET_BRAIN.md`'s "System Status" section.
